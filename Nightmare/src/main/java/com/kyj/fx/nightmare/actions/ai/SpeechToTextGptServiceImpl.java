 ...
 * 
 */
public class SpeechToTextGptServiceImpl extends SpeechToTextGptService{
	...
	public ChatBotConfig createConfig() throws Exception {
		...
		try (Connection connection = mariaDbStore.getConnection()) {
			...
			String sql = """
					select c.* from tbm_sm_cnf c where 1=1 and c.GROUP = 'OPEN_AI' AND c.KEY='TRANSLATE' \n
					""";
			...
			if (!select.isEmpty()) {
				...
				config.setProperty("apikey",
						select.get("CNF_CMF_2") == null ? "" : select.get("CNF_CMF_2").toString());
				config.setProperty("language",
						select.get("CNF_CMF_3") == null ? "" : select.get("CNF_CMF_3").toString());
				config.setProperty("prompt",
						select.get("CNF_CMF_4") == null ? "" : select.get("CNF_CMF_4").toString());
				config.setProperty("response_format",
						select.get("CNF_CMF_5") == null ? "" : select.get("CNF_CMF_5").toString());
			}
